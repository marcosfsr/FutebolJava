 ...
 * @author cefas
 */
public class JanelaBuscaJogador extends JFrame implements ActionListener{
    ...
    public void BuscaJogador(Controle x){
        c=x;
        p = new JPanel();
    ...
    public void actionPerformed(ActionEvent ae) {
        if(ae.getSource()==todos){
            texto.setText(c.ListarJogadores());
        }else if(ae.getSource()==selecionar){
            String[] v;
            v=((String)box.getSelectedItem()).split(" ");
            texto.setText(c.BuscaJogador(Integer.parseInt(v[0])-1));
        }
    }
